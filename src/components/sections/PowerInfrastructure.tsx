      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Strategic location with redundant power supply from national grid
            and renewable sources, ensuring 99.99% uptime for mission-critical
            AI workloads
          </p>
        </motion.div>
              ...
              <div className="relative h-[600px] lg:h-[700px]">
                ...
                {infrastructurePoints.map((point) => (
                  ...
                  >
                    {/* Ripple Effect */}
                    <div
                      className={`absolute -inset-4 bg-gradient-to-r ${point.color} rounded-full opacity-20 group-hover:opacity-40 transition-opacity`}
                    >
                    ...
                    {/* Icon */}
                    <div
                      className={`relative w-12 h-12 bg-gradient-to-r ${point.color} rounded-full flex items-center justify-center shadow-lg transform transition-transform group-hover:scale-110`}
                    >
                    ...
                    {/* Label */}
                    <div
                      className={`absolute top-full mt-2 left-1/2 transform -translate-x-1/2 whitespace-nowrap bg-slate-900/90 backdrop-blur-sm px-3 py-1 rounded-lg text-sm text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`}
                    >
                      {point.name}
                    </div>
                  </motion.div>
          ...
          >
            {activePoint && (
              <div className="bg-slate-800/50 backdrop-blur-sm rounded-3xl p-8 border border-slate-700/50">
                {(() => {
                  const point = infrastructurePoints.find(
                    (p) => p.id === activePoint
                  )
                  ...
                  return (
                    <>
                      <div className="flex items-center gap-4 mb-6">
                        <div
                          className={`w-16 h-16 bg-gradient-to-r ${point.color} rounded-2xl flex items-center justify-center`}
                        >
                      ...
                      <div className="space-y-4">
                        {Object.entries(point.details).map(([key, value]) => (
                          <div
                            key={key}
                            className="flex justify-between items-center p-4 bg-slate-900/50 rounded-xl border border-slate-700/30"
                          >
